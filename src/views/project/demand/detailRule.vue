        ...
        <div class="d-info">
          ...
          <div class="d-set">
            ...
            <div>
              ...
              <el-button type="primary" @click="publishOrClose('publish')">保存并发布问卷</el-button>
            </div>
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    getDemandDetail() {
      this.$http
        .post("/api/edu/projectQuestion/getOne", { surveyId: this.demandId })
        .then(res => {
          this.id = res.data.questionId;
          this.getDetail();
          this.getInfo();
        })
        .catch(msg => {});
