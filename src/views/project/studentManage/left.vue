    ...
    >
      <el-submenu :index="item.url" :key="item.url" v-for="item in menuList">
        ...
        <el-menu-item-group :key="chirld.url" v-for="chirld in item.son">
          <el-menu-item :index="chirld.url">{{ chirld.title }}</el-menu-item>
        </el-menu-item-group>
      </el-submenu>
    </el-menu>
  </el-scrollbar>
...
export default {
  ...
  data() {
    return {
      openeds: ["/studentManage", "/studentReport", "/performance"],
      routess: "/manage",
      menuActive: "/studentManage",
      menuList: [
        {
          title: "项目筹备",
          url: "/studentManage",
  ...
  computed: {
    activeRouter() {
      ...
      return this.$utils.getMenu(this.$route, 1);
    },
